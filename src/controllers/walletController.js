...
exports.getBalance = async (req, res) => {
  ...
  try {
    ...
    let result = await client.query(`SELECT balance FROM wallets WHERE user_id = $1`, [userId]);

    if (result.rows.length === 0) {
      await client.query(`INSERT INTO wallets (user_id, balance, updated_at) VALUES ($1, 0, NOW())`, [userId]);
      result = await client.query(`SELECT balance FROM wallets WHERE user_id = $1`, [userId]);
    }
...
exports.addBalance = async (req, res) => {
  ...
  try {
    ...
    if (metodo === "cartao") {
      const [userRows] = await client.query("SELECT card_token FROM usuarios WHERE id=$1", [userId]);
      if (!userRows[0]?.card_token) {
        await client.query("ROLLBACK");
        return res.status(400).json({ error: "Usuário não possui cartão cadastrado" });
      }

      await client.query(`UPDATE wallets SET balance = balance + $1, updated_at = NOW() WHERE user_id = $2`, [valor, userId]);
      await client.query(
